...
export class VitalsEstimateManager implements IVitalsEstimateManager {
  ...
  private ensureSession(sourceId: string): any {
    if (!this.sessions.has(sourceId)) {
      ...
      const config = {
        ...
        return_waveforms: this.methodConfig.supportedVitals.filter((v) =>
          v.includes('waveform')
        ),
      };
  ...
  private mapWasmResultToVitalLensResult(
    wasmResult: any,
  ...
  async processIncrementalResult(
    ...
    if (
      incrementalResult.face?.coordinates &&
      incrementalResult.face?.confidence
    ) {
      faceInput = {
  ...
  async getResult(sourceId: string): Promise<VitalLensResult> {
    const session = this.ensureSession(sourceId);
    const wasmResult = session.processJs(
      { timestamp: [], signals: {} },
      'Global'
    );
