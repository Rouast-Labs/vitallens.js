...
export interface VitalCalculationContext {
  confidence?: number[];
  timestamps?: number[];
...
/**
 * Standardized wrapper for HRV calculation to be used in the Registry.
 *
 * @param ppgWaveform - The input signal array.
 * @param fs - Sampling frequency.
 * @param metric - The specific HRV metric to calculate.
 * @param context - Context containing timestamps, confidence, and estimated HR.
 * @returns The calculated HRV value or null.
 */
export const estimateHrv = (
  ppgWaveform: number[],
  fs: number,
  metric: HRVMetric,
  ...
  // Calculate HRV metric
  const result = estimateHrvFromDetectionSequences(
    sequences,
    context.confidence || new Array(ppgWaveform.length).fill(1.0),
    fs,
    metric,
    {
