...
import { VitalLensAPIError, VitalLensAPIKeyError, VitalLensAPIQuotaExceededError } from '../../src/utils/errors';
...
const mockWebSocketClient: Partial<IWebSocketClient> = {
  ...
  sendFrames: jest.fn<
    (metadata: Record<string, any>, frames: Uint8Array, state?: Float32Array) => Promise<VitalLensAPIResponse>
  >(),
};
...
const mockResponse: VitalLensAPIResponse = {
  ...
  body: {
    vital_signs: {
      ppg_waveform: { data: [1, 2, 3], confidence: [0.9, 0.9, 0.9], note: "", unit: "" },
      respiratory_waveform: { data: [1, 2, 3], confidence: [0.9, 0.9, 0.9], note: "", unit: "" },
    },
...
describe('VitalLensAPIHandler', () => {
  const mockOptionsREST: VitalLensOptions = { apiKey: 'test-key', method: 'vitallens', requestMode: 'rest' };
  const mockOptionsWS: VitalLensOptions = { apiKey: 'test-key', method: 'vitallens', requestMode: 'websocket' };
  const mockFrame = {
    getUint8Array: jest.fn().mockReturnValue(new Uint8Array([1, 2, 3])),
    getROI: jest.fn().mockReturnValue([{ x0: 0, y0: 0, x1: 10, y1: 10 }, { x0: 0, y0: 0, x1: 10, y1: 10 }, { x0: 0, y0: 0, x1: 10, y1: 10 }]),
  ...
  it('should initialize and connect a WebSocket client', async () => {
    const handler = new VitalLensAPIHandler(mockWebSocketClient as IWebSocketClient, mockOptionsWS);
    await handler.init();
  ...
  it('should return true for REST client readiness', () => {
    const handler = new VitalLensAPIHandler(mockRestClient as IRestClient, mockOptionsREST);
    expect(handler.getReady()).toBe(true);
  });

  it('should process frames and return a valid result for WebSocket client', async () => {
    mockWebSocketClient.sendFrames = jest.fn<(metadata: Record<string, any>, frames: Uint8Array, state?: Float32Array) => Promise<VitalLensAPIResponse>>()
    ...
    expect(result).toEqual({
      face: {
        coordinates: [[0, 0, 10, 10], [0, 0, 10, 10], [0, 0, 10, 10]],
    ...
    expect(mockWebSocketClient.sendFrames).toHaveBeenCalledWith(expect.any(Object), expect.any(Uint8Array), undefined);
  });

  it('should throw an error for invalid response format', async () => {
    mockWebSocketClient.sendFrames = jest.fn<(metadata: Record<string, any>, frames: Uint8Array, state?: Float32Array) => Promise<VitalLensAPIResponse>>()
      .mockResolvedValue({statusCode: 1234, body: {face: {}, vital_signs: {}, time: [], message: ""}});
  ...
  it('should handle API key errors', async () => {
    ...
    const handler = new VitalLensAPIHandler(mockWebSocketClient as IWebSocketClient, mockOptionsWS);
    await expect(handler.process(mockFrame)).rejects.toThrow(VitalLensAPIKeyError);
  });
  ...
  it('should handle quota exceeded errors', async () => {
    ...
    mockWebSocketClient.sendFrames = jest.fn<(metadata: Record<string, any>, frames: Uint8Array, state?: Float32Array) => Promise<VitalLensAPIResponse>>()
      .mockResolvedValue(mockErrorResponse);
    const handler = new VitalLensAPIHandler(mockWebSocketClient as IWebSocketClient, mockOptionsWS);
    await expect(handler.process(mockFrame)).rejects.toThrow(VitalLensAPIQuotaExceededError);
  });
});
