...
jest.mock('../../src/core/wasmProvider', () => {
  const mockCore = {
    Session: jest.fn().mockImplementation(() => mockSessionInstance),
  };
  return {
...
describe('VitalsEstimateManager', () => {
  ...
  describe('processIncrementalResult', () => {
    it('initializes a Wasm Session and translates inputs and outputs correctly', async () => {
      ...
      const incrementalResult: VitalLensResult = {
        time: [1000, 1001],
        face: {
          coordinates: [[0, 0, 20, 20], [1, 1, 21, 21]],
          confidence: [0.9, 0.95],
        },
        vital_signs: {
          ppg_waveform: {
      ...
      mockSessionInstance.processJs.mockReturnValueOnce({
        ...
        waveforms: {
          ppg_waveform: { data: [0.5, 0.6], confidence: [0.8, 0.85], unit: 'unitless', note: 'wf note' }
        },
        vitals: {
          heart_rate: { value: 72, confidence: 0.9, unit: 'bpm', note: 'hr note' }
        },
        fps: 30.0,
        message: 'Success',
      });
      ...
      expect(result!.vital_signs.ppg_waveform).toEqual({
        data: [0.5, 0.6], confidence: [0.8, 0.85], unit: 'unitless', note: 'wf note'
      });
    });
  ...
  describe('produceBufferedResults', () => {
    it('unrolls a multi-frame result into an array of single-frame results', async () => {
      ...
      mockSessionInstance.processJs.mockReturnValueOnce({
        timestamp: [1000, 1001],
        face: { coordinates: [[0,0,1,1], [2,2,3,3]], confidence: [0.9, 0.9] },
        waveforms: {
      ...
      const results = await manager.produceBufferedResults(incrementalResult, 'source1', 'windowed');
