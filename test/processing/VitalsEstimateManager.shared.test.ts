...
describe('VitalsEstimateManager', () => {
  ...
  describe('processIncrementalResult', () => {
    beforeEach(() => {
      jest.spyOn(manager as any, 'updateWaveforms').mockImplementation(() => {});
      jest.spyOn(manager as any, 'updateTimestamps').mockImplementation(() => {});
    ...
    it('should throw an error if no waveform data is provided', async () => {
      await expect(
        manager.processIncrementalResult({ face: {}, vital_signs: {}, time: [], message: "" }, 'sourceId', 'complete')
    ...
    it('should initialize buffers and timestamps for a new sourceId', async () => {
      const incrementalResult = {
        ...
        time: [1000, 1001, 1002],
        vital_signs: {
          ppg_waveform: { data: [1, 2, 3], confidence: [0.8, 0.9, 1.0], unit: "", note: "" },
          respiratory_waveform: { data: [4, 5, 6], confidence: [0.7, 0.8, 0.9], unit: "", note: "" }
        },
        message: ""
      };
    ...
    it('should update timestamps and waveforms', async () => {
      ...
      const incrementalResult = {
        ...
        vital_signs: {
          ppg_waveform: { data: [1, 2, 3, 4, 5, 6], confidence: [0.9, 0.9, 0.9, 0.9, 0.9, 0.9], unit: "", note: "" },
          respiratory_waveform: { data: [4, 5, 6, 7, 8, 9], confidence: [0.8, 0.8, 0.8, 0.8, 0.8, 0.8], unit: "", note: "" }
        },
        face: {
          coordinates: [[0, 0, 20, 20], [0, 0, 20, 20], [0, 0, 20, 20], [0, 0, 20, 20], [0, 0, 20, 20], [0, 0, 20, 20]] as Array<[number, number, number, number]>,
  ...
  describe('getUpdatedValues', () => {
    it('should append new values when there is no overlap', () => {
      ...
      const result = manager['getUpdatedValues'](currentValues, newValues, 'incremental', 0);
    ...
    it('should handle overlap correctly and update the array', () => {
      ...
      const result = manager['getUpdatedValues'](currentValues, newValues, 'incremental', 2);
    ...
    it('should keep all values in complete mode regardless of buffer size', () => {
      ...
      manager['bufferSizePpg'] = 8;
      manager['bufferSizeResp'] = 8;
      const result = manager['getUpdatedValues'](currentValues, newValues, 'complete', 0);
  ...
  describe('getUpdatedSumCount', () => {
    it('should initialize sum and count arrays when they are empty', () => {
      ...
      const result = manager['getUpdatedSumCount'](currentBuffer, incremental, 'incremental', 10, 0);
    ...
    it('should handle overlap correctly and update sum and count', () => {
      ...
      const result = manager['getUpdatedSumCount'](currentBuffer, incremental, 'incremental', 10, 1);
    ...
    it('should keep all values in complete mode regardless of buffer size', () => {
      const currentBuffer = { sum: [1, 2, 3, 4, 5], count: [1, 1, 1, 1, 1] };
      const incremental = [6, 7, 8, 9, 10];
      const result = manager['getUpdatedSumCount'](currentBuffer, incremental, 'complete', 8, 0);
  ...
  describe('updateTimestamps', () => {
    ...
    it('should set the updated timestamps correctly in the map', () => {
      ...
      jest.spyOn(manager as any, 'getUpdatedValues').mockReturnValue([1000, 1001, 1002, 1003, 1004, 1005]);
      ...
      expect(manager['getUpdatedValues']).toHaveBeenCalledWith([1000, 1001, 1002, 1003], newTimestamps, 'incremental', 1);
    });
  });
  ...
  describe('updateFaces', () => {
    beforeEach(() => {
      manager['faces'].set('source1', {
        coordinates: [[0, 0, 20, 20], [10, 10, 30, 30]] as Array<[number, number, number, number]>,
        confidence: [0.8, 0.9]
      });
    ...
    it('should set the updated faces correctly in the map', () => {
      const newFaces = {
        coordinates: [[15, 15, 25, 25], [20, 20, 40, 40]] as Array<[number, number, number, number]>,
      ...
      jest.spyOn(manager as any, 'getUpdatedValues').mockReturnValueOnce([0.8, 0.9, 0.95, 0.99]);
  ...
  describe('updateWaveforms', () => {
    beforeEach(() => {
      ...
      manager['waveformNotes'].set('source1', { ppg: 'Initial PPG note', resp: 'Initial RESP note' });
    });

    it('should update waveform buffers and notes correctly', () => {
      const newVitals = {
        ppg_waveform: { data: [4, 5, 6], confidence: [0.95, 0.96, 0.97], unit: "", note: 'Updated PPG note' },
        respiratory_waveform: { data: [7, 8, 9], confidence: [0.75, 0.76, 0.77], unit: "", note: 'Updated RESP note' }
      };
      
      jest.spyOn(manager as any, 'getUpdatedSumCount')
        .mockReturnValueOnce({ sum: [1, 2, 3, 4, 5, 6], count: [1, 1, 1, 1, 1, 1] }) // For ppgData
        .mockReturnValueOnce({ sum: [0.8, 0.9, 1.0, 0.95, 0.96, 0.97], count: [1, 1, 1, 1, 1, 1] }) // For ppgConf
        .mockReturnValueOnce({ sum: [4, 5, 6, 7, 8, 9], count: [1, 1, 1, 1, 1, 1] }) // For respData
        .mockReturnValueOnce({ sum: [0.7, 0.8, 0.9, 0.75, 0.76, 0.77], count: [1, 1, 1, 1, 1, 1] }); // For respConf
      ...
      expect(updatedWaveforms).toEqual({
        ...
        ppgConf: { sum: [0.8, 0.9, 1.0, 0.95, 0.96, 0.97], count: [1, 1, 1, 1, 1, 1] },
        respData: { sum: [4, 5, 6, 7, 8, 9], count: [1, 1, 1, 1, 1, 1] },
        respConf: { sum: [0.7, 0.8, 0.9, 0.75, 0.76, 0.77], count: [1, 1, 1, 1, 1, 1] }
      });
  ...
  describe('assembleResult', () => {
    beforeEach(() => {
      ...
      manager['waveformNotes'].set('source1', { ppg: 'PPG note', resp: 'RESP note' });
      manager['timestamps'].set('source1', [1001, 1002, 1003, 1004, 1005]);
      manager['faces'].set('source1', {
        coordinates: [[0, 0, 20, 20], [0, 0, 20, 20], [0, 0, 20, 20], [10, 10, 30, 30], [15, 15, 25, 25]],
    ...
    it('should assemble an incremental result correctly', async () => {
      const incrementalResult = {
        time: [1004, 1005, 1006],
        face: {
          coordinates: [[20, 20, 40, 40], [20, 20, 40, 40], [25, 25, 50, 50]],
          confidence: [0.91, 0.92, 0.93]
        },
        vital_signs: {
          ppg_waveform: { data: [1, 2, 3], confidence: [0.7, 0.8, 0.9] },
          respiratory_waveform: { data: [4, 5, 6], confidence: [0.6, 0.7, 0.8] }
        }
      };
      ...
      const result = await manager['assembleResult']('source1', 'incremental', 2, incrementalResult, 1);
    ...
    it('should assemble an aggregated result correctly', async () => {
      ...
      expect(result).toEqual({
        ...
        face: {
          coordinates: [[0, 0, 20, 20], [0, 0, 20, 20], [10, 10, 30, 30], [15, 15, 25, 25]],
      ...
      expect(manager['getCurrentFps']).toHaveBeenCalledWith('source1', manager['bufferSizeAgg']);
    });
