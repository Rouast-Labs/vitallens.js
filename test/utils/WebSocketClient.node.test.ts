...
describe("WebSocketClient (Node)", () => {
  ...
  it("should connect successfully", async () => {
    ...
    (WebSocket as unknown as jest.Mock).mockImplementation(() => mockSocket as any);


    const connectPromise = client.connect();

    // Trigger the "open" event
    const openHandler = mockSocket.on.mock.calls.find(([event]) => event === "open")?.[1];
    openHandler?.();
  ...
  it("should handle connection errors", async () => {
    ...
    await expect(connectPromise).rejects.toThrow("WebSocket failed to connect: Mock error");
