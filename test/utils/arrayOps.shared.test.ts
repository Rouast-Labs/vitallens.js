...
describe("mergeFrames", () => {
  ...
  it("merges frames correctly from Frame with keepTensor", async () => {
    const frame1 = await Frame.fromTensor(tf.tensor([1, 2]), false, [1], [{ x0: 0, y0: 0, x1: 2, y1: 2 }]);
    const frame2 = await Frame.fromTensor(tf.tensor([3, 4]), false, [2], [{ x0: 1, y0: 1, x1: 3, y1: 3 }]);
  ...
  it("merges frames correctly from Frame with keepTensor", async () => {
    const frame1 = await Frame.fromTensor(tf.tensor([1, 2]), true, [1], [{ x0: 0, y0: 0, x1: 2, y1: 2 }]);
    const frame2 = await Frame.fromTensor(tf.tensor([3, 4]), true, [2], [{ x0: 1, y0: 1, x1: 3, y1: 3 }]);
