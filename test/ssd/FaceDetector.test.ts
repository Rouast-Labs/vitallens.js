...
import { FaceDetector } from '../../src/ssd/FaceDetector';
...
describe('FaceDetector', () => {
  ...
  beforeAll(() => {
    ...
    const mockGraphModel = {
      execute: jest.fn().mockImplementation(() => {
        // Return a mock tensor output for `execute`
        return [
          tf.tensor3d([
            [
    ...
    // Mock nonMaxSuppression to simulate selected indices (for synchronous version)
    jest.spyOn(tf.image, 'nonMaxSuppression').mockImplementation(() => {
      return tf.tensor1d([0]); // Assume the first box is selected
    });
  
    // Initialize the FaceDetector
    faceDetector = new FaceDetector(1, 0.5, 0.3);
  });
  ...
  it('should detect faces and return ROIs', async () => {
    ...
    expect(tf.image.nonMaxSuppression).toHaveBeenCalled();
  ...
  it('should throw an error if the model is not loaded', async () => {
    const uninitializedDetector = new FaceDetector(1, 0.5, 0.3);
    const mockData = new Float32Array(224 * 224 * 3).fill(0);
    const mockTensor = tf.tensor4d(mockData, [1, 224, 224, 3]);
    const frame = new Frame(mockTensor, [0]);
